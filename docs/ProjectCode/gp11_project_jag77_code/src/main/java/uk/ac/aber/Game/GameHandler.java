 ...
 * objects for the uk.ac.aber.Game control to use.
 *
 * @author asb20
...
public class GameHandler {
    ...
    public boolean ContinueGame() throws IOException {
        ...
        if (this.isGameValid()) {
            ...
            // not a fan of the fact that players
            // are loaded here then need to be retrieved.
            // aka data in two different places?
     ...
     * Creates the JSON necessary from a uk.ac.aber.Game object and stores it in the correct place locally.
     *
     * @param gm The uk.ac.aber.Game object that contains the data for the current ongoing game
     *
     * @see Game
     * @see #gameConfig
     */
     ...
     * Stores the ongoing game from the local disk as a uk.ac.aber.Game object.
     *
     * @return Returns the uk.ac.aber.Game object created from local files
     * @see Game
     * @see #gameConfig
     */
    ...
    public boolean savePlayer(Player pl) {
        ...
        if (f.exists() && !f.isDirectory()) {
            System.out.println("Trying to save players");
            Gson gson = new GsonBuilder().setPrettyPrinting().create();
